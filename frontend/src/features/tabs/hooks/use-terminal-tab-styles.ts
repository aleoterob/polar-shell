...
export function useTerminalTabStyles() {
  const chromeBg = "bg-[#262335]";

  const barClassName = cn(
    "flex h-10 min-w-0 shrink-0 items-end gap-1 overflow-x-auto overflow-y-hidden border-b border-terminal-tab-border px-2 pt-1",
    chromeBg,
  );

  const listClassName = cn(
    "h-auto gap-0.5 rounded-none border-0 p-0 shadow-none",
    chromeBg,
  );
  ...
  const triggerClassName = cn(
    ...
    "data-active:z-[1] data-active:bg-[#262335] data-active:text-foreground",
    "data-active:border-terminal-tab-border data-active:border-b-[#0c0c0c]",
    "data-active:shadow-none dark:data-active:!border-terminal-tab-border dark:data-active:bg-[#262335]",
  );
